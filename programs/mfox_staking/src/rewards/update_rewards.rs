...
pub fn update_rewards(stake_pool: &mut StakePool, stake_entry: &mut StakeEntry) -> ProgramResult {
    if stake_pool.last_update_timestamp < stake_pool.end_timestamp {
...
fn update_stake_pool(stake_pool: &mut StakePool) -> ProgramResult {
    let mut now = Clock::get().unwrap().unix_timestamp;
    if now > stake_pool.end_timestamp {
        now = stake_pool.end_timestamp;
    }
